...
def readPosition(ID):
	...
	header = serial_port.read(3)
	print('Received header: ' + ":".join("{:02x}".format(ord(c)) for c in header))
	body = serial_port.read(2)
	print('Received body (length&error): ' + ":".join("{:02x}".format(ord(c)) for c in body))
	position = serial_port.read(1)
	print('Position: ' + ":".join("{:02d}".format(ord(c)) for c in position))
	r_checksum = serial_port.read(1)
	print('Received checksum: ' + ":".join("{:02x}".format(ord(c)) for c in r_checksum))
	return

def motor_1_move():
	pub = rospy.Publisher('motor_1_move', Float64, queue_size=10)
	rospy.init_node('Dynamixel_python', anonymous=True)
	rate =rospy.Rate(10)
